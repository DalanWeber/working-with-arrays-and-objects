...
let myStrongest = monstersInYourPocket.filter((cp)=> {
  return cp['CP'] > 200
})
...
let orderTotals = orders.map((price) =>{
    return price['price'] + (price['price'] *price['tax'])
...
let filter = purchases.filter((owner)=>{
  return owner['owner'] ===  'Bob'
})

let bobsTotal = filter.reduce((acc,price) =>{
